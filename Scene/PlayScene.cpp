...
#include "Enemy/CentauroEnemy.hpp"
...
#include "Turret/GhostTurret.hpp"
...
void PlayScene::Update(float deltaTime) {
    ...
    for (int i = 0; i < SpeedMult; i++) {
        ...
        if (enemyWaveData.empty()) {
            if (EnemyGroup && EnemyGroup->GetObjects().empty()) {
                // Free resources.
                Engine::GameEngine::GetInstance().tmp_record.score = money;
                Engine::GameEngine::GetInstance().tmp_record.time =
                    time(nullptr);
        ...
        switch (current.first) {
            ...
            case 3:
                ...
                break;
                // TODO: [CUSTOM-ENEMY]: You need to modify
                // 'Resource/enemy1.txt', or 'Resource/enemy2.txt' to spawn the
                // 4th enemy.
                //         The format is "[EnemyId] [TimeDelay] [Repeat]".
                // TODO: [CUSTOM-ENEMY]: Enable the creation of the enemy.
            case 4:
                EnemyGroup->AddNewObject(
                    enemy = new CentauroEnemy(SpawnCoordinate.x,
                                              SpawnCoordinate.y));
                break;
            default:
                continue;
...
void PlayScene::OnKeyDown(int keyCode) {
    ...
    if (keyCode == ALLEGRO_KEY_TAB) {
        ...
        if (keyCode == ALLEGRO_KEY_ENTER && keyStrokes.size() == code.size()) {
            ...
            for (int c : code) {
                if (!((*it == c) || (c == ALLEGRO_KEYMOD_SHIFT &&
                                     (*it == ALLEGRO_KEY_LSHIFT ||
                                      *it == ALLEGRO_KEY_RSHIFT))))
                    return;
                ++it;
            }
...
void PlayScene::ConstructUI() {
    ...
    btn = new TurretButton(
        ...
        Engine::Sprite("play/turret-5.png", 1522, 136, 0, 0, 0, 0), 1522, 136,
        GhostTurret::Price);
    btn->SetOnClickCallback(std::bind(&PlayScene::UIBtnClicked, this, 3));
    UIGroup->AddNewControlObject(btn);
    // TODO: [CUSTOM-TURRET]: Create a button to support constructing the
    // turret.
...
void PlayScene::UIBtnClicked(int id) {
    ...
    else if (id == 3 && money >= GhostTurret::Price)
        preview = new GhostTurret(0, 0);
    if (!preview)
        return;
