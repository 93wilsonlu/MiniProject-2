...
namespace Engine {
	void GameEngine::initAllegro5() {
		...
		std::string name;
		while (file >> name >> tmp_record.score >> tmp_record.time) {
	...
	void GameEngine::destroy() {
		...
		for (auto& record : scoreboard) {
			file << record.name << " " << record.score << " " << record.time << '\n';
		}
		file.close();
	}
	...
	void GameEngine::updateScoreboard(std::string name) {
		tmp_record.name = name;
		int scoreboard_len = scoreboard.size();
		for(int i = 0; i < scoreboard_len; i++){
			if (scoreboard[i].name == name) {
				scoreboard[i] = tmp_record;
				return;
			}
		}
		scoreboard.push_back(tmp_record);
		scoreboard_len++;
		for(int i = scoreboard_len - 1; i && scoreboard[i - 1] < scoreboard[i]; i--){
			std::swap(scoreboard[i - 1], scoreboard[i]);
		}
	}
}
